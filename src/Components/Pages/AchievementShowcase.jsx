...
const AchievementShowcase = () => {
  ...
  const IconWrapper = ({ children, bgColor }) => (
    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${bgColor} transition-transform duration-300 transform hover:scale-110`}>
      {children}
    </div>
  );
  ...
  return (
    <div className="lg:mx-auto bg-white lg:container min-h-screen flex flex-col">
      ...
      <div className={`flex-1 flex ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
        {/* Sidebar for desktop */}
        <aside className={`hidden md:flex flex-col w-64 p-4 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'} sticky top-0 h-screen`}>
        ...
        {/* Main content */}
        <main className={`flex-1 p-8 ${isDarkMode ? 'bg-gray-900' : 'bg-white'} min-h-full`}>
                  ...
                  {menuItems.map((item, index) => (
                    <button
                      ...
                      className={`
                        ...
                        ${isDarkMode ? 'text-white hover:bg-gray-700' : 'text-purple-700 hover:bg-purple-100'}
